...
namespace Nightingale.ViewModels
{
    public class RequestViewModel : ObservableBase
    {
        ...
        {
            _workspaceTreeModifier = workspaceListModifier ?? throw new ArgumentNullException(nameof(workspaceListModifier));
            _tabCollectionContainer = tabCollectionContainer ?? throw new ArgumentNullException(nameof(tabCollectionContainer));
        ...
        { 
            Analytics.TrackEvent(Telemetry.TabContext, Telemetry.Props(Telemetry.TabContextRename));
            await _workspaceTreeModifier.EditItemAsync(item); 
        }

        private async void DuplicateTab(Item item)
        {
            Analytics.TrackEvent(Telemetry.TabContext, Telemetry.Props(Telemetry.TabContextDuplicate));
            await _tabCollectionContainer.DuplicateTabAsync(item);
        }

        private void CloseOthers(Item item)
        {
            Analytics.TrackEvent(Telemetry.TabContext, Telemetry.Props(Telemetry.TabContextCloseOthers));
            _tabCollectionContainer.RemoveAllButThis(item);
        }

        private void CloseTab(Item item)
        {
            Analytics.TrackEvent(Telemetry.TabContext, Telemetry.Props(Telemetry.TabContextClose));

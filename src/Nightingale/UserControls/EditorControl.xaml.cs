...
public sealed partial class EditorControl : UserControl
{
    ...
    private async void ActiproEditor_KeyDown(object sender, Windows.UI.Xaml.Input.KeyRoutedEventArgs e)
    {
        if (KeyboardState.IsCtrlKeyPressed() && e.Key == VirtualKey.F)
        {
            ...
            SearchBox.SelectAll();
            App.Services.GetRequiredService<ITelemetry>().TrackEvent(Telemetry.SearchOpened);
            e.Handled = true;
        }
    ...
    private void RefreshHighlights(bool reverse, string text = "")
    {
        ...
        App.Services.GetRequiredService<ITelemetry>().TrackEvent(Telemetry.SearchPerformed, new Dictionary<string, string>
        {
    ...
    {
        App.Services.GetRequiredService<ITelemetry>().TrackEvent(Telemetry.SearchClosed);
    ...
    private void ToggleWrapClicked()
    {
        ...
        App.Services.GetRequiredService<ITelemetry>().TrackEvent(Telemetry.WordWrapClicked);
    }
    ...
    private void BeautifyClicked()
    {
        ...
        App.Services.GetRequiredService<ITelemetry>().TrackEvent(Telemetry.BeautifyClicked);
    }
}

...
public sealed partial class EditorControl : UserControl
{
    ...
    public static DependencyProperty SyntaxProperty = DependencyProperty.Register(
        nameof(Syntax),
        typeof(SyntaxType),
        typeof(EditorControl),
        new PropertyMetadata(SyntaxType.Plain, OnSyntaxChanged));

    private static void OnSyntaxChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        if (d is EditorControl c)
        {
            c.UpdateSyntaxType(e.NewValue as SyntaxType?);
        }
    }
    ...
    private void UpdateSyntaxType(SyntaxType? newSyntax)
    {
        NewEditor.HighlightingLanguage = newSyntax switch
        {
